...
pipeline {
    ...
    environment {
        ...
        AWS_SECRET_ACCESS_KEY = credentials('AWS_SECRET_ACCESS_KEY_ID') // Secret Access Key from Jenkins credentials
    ...
    stages {
        ...
        stage('Deploy to S3') {
            steps {
                ...
                script {
                    sh "aws s3 cp public/index.html s3://jenkinstf-ec2-static-bucket2023/"
                    sh "aws s3 cp public/error.html s3://jenkinstf-ec2-static-bucket2023/"
                }
            }
        }
        ...
        stage('Set Bucket Policy') {
            steps {
                ...
                script {
                    sh "aws s3api put-bucket-policy --bucket jenkinstf-ec2-static-bucket2023 --policy file://policy.json"
                }
            }
        }
    }
